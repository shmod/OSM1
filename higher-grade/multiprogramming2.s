...
boot:
	...
	la $t1, job_gets		#Specify the job we want!
	sw $t1, 0($t0)
...
PRESS_MMIO_KEYS:	.asciiz "Type your characters in the MMIO Simulator keyboard input area!... \n"
...
job_gets_infinite_loop:
	...
	# Use the built in version of the gets system call	Is this allowed?
	la $a0, STRING_ARRAY
	addi $a1, $zero, 5	# Our buffer size
	li $v0, 8		# System call code (8) read_char.
	teqi $zero, 0    
...
__kernel:
	...
	# Now the value of $at can be saved to memory. 
        sw $k1, __at      	   
      	...
      	# The registers $k0 and $k1 should o y be used in the exception
...
TODO_3: # Jump to label __system_call_getc for system call code 12.
	...
	li $t0, 12
   	...
   	# Jump to label __system_call_gets for system call code 8
   	li $t0, 8
   	beq $v0, $t0, __system_call_gets
...
__kbd_interrupt:
	...
	# We check to see if job_1 waiting job is gets
	
	addi $t0, $zero, 24
	lw $t1, __job_1_context($t0)
	beq $t1, 1, __gets_system_call_pending
...
__gets_system_call_pending:
	...
	# A job is waiting for input. If the keyboard input is 0000 resume to job
	li $k1, 0xffff0004  
	lw $v0, 0($k1)			# receiver data
	
	# Check how many bytes are free in the buffer
	sll $k0, $k0, 2   # jid * 4
        lw $t2, __context_array($k0)   
          
	lw $a1,  12($t2) # $a1
	move $t1, $a1			# where does the new value go?
	
	# update remaining space on buffer
	addi $a1, $a1, -1		
	sw $a1, 12($t2)
	
	lw $t1, __constant
	sb $v0, STRING_ARRAY($t1)
	addi $t1, $t1, 1
	sw $t1, __constant
	
	beqz $a1, __gets_finish
	#NOW WE'VE SAVED THE INCOMING CHARACTER WE WANT TO RETURN TO OUR JOB THAT WE ARE RUNNING
	mfc0 $k0, $12
...
__gets_finish:
	...
	# Reset constant and finish string
	addi $t0, $zero, 0
	lw $t1, __constant
	sb $t0, STRING_ARRAY($t1)
	sw $t0, __constant
